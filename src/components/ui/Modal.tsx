        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              className={`bg-surface rounded-card shadow-float w-full ${sizeClasses[size]} overflow-hidden`}
            >
              {/* Header */}
              {title && (
                <div className={`p-4 border-b border-border flex items-center justify-between ${headerClassName || ""}`}>
                  <h3 className={`text-title2 ${headerClassName ? "" : "text-fg"}`}>{title}</h3>
                  <button
                    onClick={onClose}
                    className={`p-1 rounded-full transition ${headerClassName ? "hover:bg-white/20 text-current" : "hover:bg-surface-2 text-fg-secondary"}`}
                  >
                    <X size={24} />
                  </button>
                </div>
