      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-50 glass rounded-t-sheet shadow-float max-h-[85vh] flex flex-col touch-none select-none safe-area-bottom border-b-0"
            style={{ willChange: "transform" }}
          >
            {/* Handle para arrastrar - visible and accessible */}
            <div className="flex justify-center pt-3 pb-2 cursor-grab active:cursor-grabbing">
              <div className="w-12 h-1.5 bg-fg-tertiary/40 rounded-full" />
            </div>

            {/* Header */}
            {title && (
              <div className={`px-4 pb-3 border-b border-border flex items-center justify-between ${headerClassName || ""}`}>
                <h3 id={titleId} className={`text-title2 ${headerClassName ? "" : "text-fg"}`}>{title}</h3>
                <motion.button
                  whileTap={{ scale: 0.92 }}
                  transition={{ type: "tween", duration: 0.1 }}
                  onClick={onClose}
                  className={`p-2 rounded-full transition-colors touch-manipulation ${headerClassName ? "hover:bg-white/20 active:bg-white/30 text-current" : "hover:bg-surface-2 text-fg-secondary"}`}
                  aria-label="Cerrar"
                >
                  <X size={24} />
                </motion.button>
