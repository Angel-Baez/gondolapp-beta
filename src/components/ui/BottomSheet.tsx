 ...
 * - Máximo 90vh de altura con scroll interno
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-dark-surface rounded-t-3xl shadow-native-xl max-h-[90vh] flex flex-col touch-none select-none safe-area-bottom transition-colors"
          ...
          >
            ...
            {/* Header */}
            {title && (
              <div className={`px-5 pb-4 border-b border-gray-100 dark:border-dark-border flex items-center justify-between ${headerClassName || ""}`}>
                <h3 id={titleId} className={`text-lg font-bold ${headerClassName ? "" : "text-gray-900 dark:text-gray-100"}`}>{title}</h3>
                <motion.button
                  whileTap={{ scale: 0.9 }}
                  transition={{ type: "tween", duration: 0.1 }}
                  onClick={onClose}
                  className={`p-2 rounded-xl transition-colors touch-manipulation min-h-[44px] min-w-[44px] flex items-center justify-center ${headerClassName ? "hover:bg-white/20 active:bg-white/30 text-current" : "hover:bg-gray-100 dark:hover:bg-dark-card active:bg-gray-200 dark:active:bg-dark-border text-gray-500 dark:text-gray-400"}`}
                  aria-label="Cerrar"
                >
                  <X size={20} />
                </motion.button>
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain p-5 native-scroll">
              {children}
            </div>
          </motion.div>
        </>
